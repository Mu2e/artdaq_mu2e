	...
	switch (type)
	{
		...
		default:
			throw art::Exception(art::errors::Configuration)
				<< "Unknown board type " << type << " (" << mu2e::fragmentTypeToString(type) << ").\n";
	};
}
}  // namespace

mu2e::OverlayTest::OverlayTest(fhicl::ParameterSet const& ps)
	: CommandableFragmentGenerator(ps), nADCcounts_(ps.get<size_t>("nADCcounts", 600000)),  // No longer used as adc counts are based on data read from the DTC simulator
...
{
	std::vector<artdaq::Fragment::type_t> const ftypes = {FragmentType::TRK, FragmentType::CAL, FragmentType::CRV,
														  FragmentType::DBG};
	...
	if (fragment_type_ == mu2e::FragmentType::DBG) {
		theCFO_ =
			new DTCLib::DTCSoftwareCFO(theInterface, false, debugPacketCount_, debugType_, stickyDebugType_, true, false);
	}
	else
	{
...
bool mu2e::OverlayTest::getNext_(artdaq::FragmentPtrs& frags)
{
	...
	mode_ = theInterface->ReadSimMode();
	if (mode_ != DTCLib::DTC_SimMode_Disabled && mode_ != DTCLib::DTC_SimMode_Loopback &&
		mode_ != DTCLib::DTC_SimMode_NoCFO && mode_ != DTCLib::DTC_SimMode_ROCEmulator) {
		TLOG_INFO("DTCOverlayTest") << "Using Simulated DTC in " << DTCLib::DTC_SimModeConverter(mode_).toString()
									<< " mode" << TLOG_ENDL;
	}
	...
	for (size_t curDataIdx = 0; curDataIdx < data.size(); curDataIdx++) {
		DTCLib::DTC_DataHeaderPacket packet =
			DTCLib::DTC_DataHeaderPacket(DTCLib::DTC_DataPacket(data[curDataIdx].blockPointer));
		totalNumPackets += packet.GetPacketCount() + 1;

		//    std::cout << "ERR: curDataIdx: " << curDataIdx << " packet.GetPacketCount()+1: " << packet.GetPacketCount()+1
		//    << std::endl << std::flush;
	...
	while (!data.empty()) {
		DTCLib::DTC_DataHeaderPacket packet =
			DTCLib::DTC_DataHeaderPacket(DTCLib::DTC_DataPacket(data[dataIdx].blockPointer));

		std::cout << "================ DATA INDEX " << dataIdx << "/" << data.size() - 1 << " ("
				  << packet.GetPacketCount() + 1 << " packets total) ================" << std::endl;
		...
		std::cout << "\t"
				  << "PacketCount: " << packet.GetPacketCount() << std::endl;

		if (packet.GetPacketCount() > 0) {
			std::cout << "\t"
					  << "DumpingDataPackets: " << std::endl;
		}
		...
		for (int packetNum = 0; packetNum < packet.GetPacketCount() + 1; packetNum++) {
			...
			for (int wordNum = 0; wordNum < 16; wordNum += 2) {  // 16 words per DTC packet
				int fragPos = ((numPacketsRecorded + packetNum) * 16 + wordNum) /
							  2;         // The index of the current 16 bit adc_t value in the fragment
				if (wordNum % 2 == 0) {  // Not strictly necessary since we increment by 2
					...
					//	  *(newfrag->dataBegin()+fragPos) = combined; // Set the appropriate adc_t entry in the fragment to the
					//combined uint16_t
				}
			}
			data_packets_read_++;
