...
art::ArtFragmentsFromDTCEvents::ArtFragmentsFromDTCEvents(const art::EDProducer::Table<Config>& config) :
  ...
  makeSTMFrag_(config().makeSTMFrag()){
    ...
    if (config().makeCaloFrag() > 0) { produces<std::vector<mu2e::CalorimeterFragment> >("Calorimeter");}
    if (config().makeTrkFrag()  > 0) { produces<std::vector<mu2e::TrackerFragment> >("Tracker");}
    if (config().makeCRVFrag()  > 0) { produces<std::vector<mu2e::CRVFragment> >("CosmicVeto");}
...
void art::ArtFragmentsFromDTCEvents::produce(Event& event) {
  ...
  if (makeCaloFrag_ > 0)  {event.put(std::move(caloFragColl), "Calorimeter");}
  if (makeTrkFrag_  > 0)  {event.put(std::move(trkFragColl), "Tracker" );}
  if (makeCRVFrag_  > 0)  {event.put(std::move(crvFragColl), "CosmicVeto" );}
