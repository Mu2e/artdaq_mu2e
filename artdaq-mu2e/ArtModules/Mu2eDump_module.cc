...
void mu2e::Mu2eDump::analyze(art::Event const& evt)
{
	...
	if (raw.isValid()) {
		...
		std::cout << std::dec << "Run " << evt.run() << ", subrun " << evt.subRun() << ", event " << eventNumber << " has "
				  << raw->size() << " mu2eFragment(s)" << std::endl;
		...
		for (size_t idx = 0; idx < raw->size(); ++idx) {
			...
			if (frag.hasMetadata()) {
				...
				mu2eFragment::Metadata const* md = frag.metadata<mu2eFragment::Metadata>();
			...
			//			  auto dhpacket =
			//DTCLib::DTC_DataHeaderPacket(DTCLib::DTC_DataPacket((uint8_t*)bb.dataBegin())); 			  if
			//(print_json_)
			//			    {
			...
			//
			//			  std::cout << std::endl;
			//			  int packetMax = max_DataPackets_to_show_ > dhpacket.GetPacketCount() ?
			//dhpacket.GetPacketCount() : max_DataPackets_to_show_; 			  for (int ii = 0; ii < packetMax; ++ii)
			//			    {
			//			      // Packets are 16 bytes. mu2eFragment::Header::data_t is a uint64_t. Therefore,
			//each packet is an offset of 2!
			...
			if (dmp.GetPacketType() == DTCLib::DTC_PacketType_DataHeader) {
				...
				std::cout << "DTC_DataHeaderPacket:" << std::endl;
				//			  auto ddhp =
				//DTCLib::DTC_DTCHeaderPacket(DTCLib::DTC_DataPacket((uint8_t*)bb.dataBegin()));
				...
				while (offset < ddhp.GetByteCount()) {
					...
					std::cout << std::endl;
					int packetMax = max_DataPackets_to_show_ > dhpacket.GetPacketCount() ? dhpacket.GetPacketCount()
																						 : max_DataPackets_to_show_;
					for (int ii = 0; ii < packetMax; ++ii) {
						// Packets are 16 bytes. mu2eFragment::Header::data_t is a uint64_t. Therefore, each packet is an offset of
						// 2!
