...
mu2e::DTCEventVerifier::DTCEventVerifier(const art::EDFilter::Table<Config>& config)
	: art::EDFilter{config}, diagLevel_(config().diagLevel()), nDTCs_(config().nDTCs()), skipCheck_(config().skipCheck()), isFirstEvent_(true)
{
...
bool mu2e::DTCEventVerifier::filter(art::Event& event)
{
	...
	for (const auto& frag : fragments)
	{
		...
		auto event = &data;
		if (diagLevel_ > 0) ostr << "Event tag:\t"
								 << "0x" << std::hex << std::setw(4) << std::setfill('0') << event->GetEventWindowTag().GetEventWindowTag(true) << std::endl;
