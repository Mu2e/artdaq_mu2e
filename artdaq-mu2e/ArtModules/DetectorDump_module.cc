...
#include "artdaq-core-mu2e/Overlays/CalorimeterFragmentReader.hh"
#include "artdaq-core-mu2e/Overlays/CosmicVetoFragmentReader.hh"
#include "artdaq-core-mu2e/Overlays/DetectorFragment.hh"
#include "artdaq-core-mu2e/Overlays/FragmentType.hh"
#include "artdaq-core-mu2e/Overlays/TrackerFragmentReader.hh"
...
private:
	...
	struct conn_holder
	{
		conn_holder(mongocxx::database&& arg)
			: db(std::move(arg)) {}
	...
	struct coll_holder
	{
		coll_holder(mongocxx::collection&& arg)
			: coll(std::move(arg)) {}
...
mu2e::DetectorDump::DetectorDump(fhicl::ParameterSet const& pset)
	: EDAnalyzer(pset), raw_data_label_(pset.get<std::string>("raw_data_label")), fragment_type_string_(pset.get<std::string>("frag_type")), fragment_type_(toFragmentType(pset.get<std::string>("frag_type"))), max_DataPackets_to_show_(static_cast<uint16_t>(pset.get<int>("max_DataPackets_to_show", 0))), print_json_(pset.get<bool>("print_json", false)), print_packet_format_(pset.get<bool>("print_packet_format", true))
{
...
#pragma GCC diagnostic pop
	...
	std::cout << "\t"
			  << "Removing events from database..." << std::endl;
...
#pragma GCC diagnostic pop
	auto variables_doc = document{} << "type"
									...
									<< "CAL.numSamples.type"
									<< "I"
									<< "CAL.ADCintegral.type"
									<< "I"

									<< "CAL.crystalID.min" << 0 << "CAL.apdID.min" << 0 << "CAL.time.min" << 0
									<< "CAL.numSamples.min" << 0 << "CAL.ADCIntegral.min" << 0

									<< "CAL.crystalID.max" << 10000 << "CAL.apdID.max" << 2 << "CAL.time.max" << 100000
									<< "CAL.numSamples.max" << 30 << "CAL.ADCIntegral.max" << 1000
									...
									<< "HEADER.rocID.min" << 0 << "HEADER.packetType.min" << 0

									<< "HEADER.rocID.max" << 5 << "HEADER.packetType.max" << 10
	...
	std::cout << "\t"
			  << "Done adding variable list to database" << std::endl;
}
...
void mu2e::DetectorDump::analyze(art::Event const& evt)
{
	...
	if (raw.isValid()) {
		...
		std::cout << std::dec << "Run " << evt.run() << ", subrun " << evt.subRun() << ", event " << eventNumber << " has "
				  << raw->size() << " DetectorFragment(s)" << std::endl;
			...
			switch (fragment_type_)
			{
				...
				case mu2e::FragmentType::CAL:
				{
					...
					for (size_t blockIdx = 0; blockIdx < bb->numDataBlocks(); blockIdx++) {
						...
						std::vector<mu2e::DetectorFragment::adc_t> CAL_adcVec =
							(dynamic_cast<CalorimeterFragmentReader*>(bb))->calorimeterADC();
						int CAL_ADCintegral = 0;
						for (size_t i = 0; i < CAL_adcVec.size(); i++) {
							CAL_ADCintegral += CAL_adcVec[i];
						}

						auto event_doc = document{} << "epoch" << epoch << "type"
													<< "event"
													//	        << "system"                  << "CAL"

													<< "CAL.crystalID" << CAL_crystalID << "CAL.apdID" << CAL_apdID << "CAL.time"
													<< CAL_time << "CAL.numSamples" << CAL_numSamples << "CAL.ADCintegral"
													<< CAL_ADCintegral

													<< "HEADER.rocID" << HEADER_rocID << "HEADER.packetType" << HEADER_packetType
...
#pragma GCC diagnostic pop
			...
			//      //      auto dhpacket =
			//      DTCLib::DTC_DataHeaderPacket(DTCLib::DTC_DataPacket((uint8_t*)*(bb->dataBegin()))); auto dhpacket =
			//      DTCLib::DTC_DataHeaderPacket(DTCLib::DTC_DataPacket((uint16_t*)*(bb->dataBegin()))); if(print_json_) {
			//      std::cout << dhpacket.toJSON() << std::endl; } if(print_packet_format_) {std::cout <<
			//      dhpacket.toPacketFormat() << std::endl; } std::cout << std::endl;

			//      std::cout << std::endl;
			//      int packetMax = max_DataPackets_to_show_ > bb->hdr_packet_count() ? bb->hdr_packet_count() :
			//      max_DataPackets_to_show_; for(int ii = 0; ii < packetMax; ++ii) {
