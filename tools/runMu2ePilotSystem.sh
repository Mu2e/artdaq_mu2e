...
if [[ "x$USER" == "xmu2edaq" ]]; then
  mkdir -p /scratch-mu2edaq01/mu2edaq/daqlogs/cron
  #exec 2>&1
  #exec > >(tee /home/mu2edaq/daqlogs/cron/runMu2ePilotSystem_${startTime}.log)
  exec 2>&1 >/scratch-mu2edaq01/mu2edaq/daqlogs/cron/runMu2ePilotSystem_${startTime}.log
fi
...
cd /scratch-mu2edaq01/mu2edaq/daqlogs/cron
